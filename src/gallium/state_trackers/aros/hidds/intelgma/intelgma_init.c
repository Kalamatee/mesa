...
static BOOL probe_monitor(struct g45staticdata *sd, uint32_t port)
{
        ...
        bug("[IntelGMA] Attempting to detect connected monitor\n");
        sd->DDCPort = port;
        bug("[IntelGMA] Probing GPIO%c\n", 'A' + (port - G45_GPIOA)/4);
        ...
        if (i2c)
        {
                if (HIDD_I2C_ProbeAddress(i2c, 0xa1))
                {
                        ...
                        if (obj)
                        {
                                                ...
                                                edid[4] == 0xff && edid[5] == 0xff && edid[6] == 0xff && edid[7] == 0)
                                {
                                        ...
                                        if (edid[0x14] & 0x80)
                                        {
                                                bug("[IntelGMA]       Digital device\n");
                                        }
                                        else
                                        {
                                                bug("[IntelGMA]       Analog device\n");
                                        }
                                }
                ...
                else bug("[IntelGMA]   No I2C device found\n");
...
static BOOL claim_device(OOP_Object *pciDevice)
{
    if (!HIDD_PCIDevice_Obtain(pciDevice, "IntelGMA"))
    {
        /* 
           NB: for now we just send #3 to Tell vga text mode debug output to die
           however we will at some point want to fix this so it checks if an existing driver is
           using the same regions for its framebuffer as we own -> if so ask it nicely
    ...
    AROS_UFHA(APTR,         message,    A1))
{
    ...
    if (sd->forced || IsCompatible(ProductID))
    {
        ...
        if (claim_device(pciDevice))
        {
            ...
            D(bug("[IntelGMA] GATT space for %d entries\n", gatt_size / 4));

            D(bug("[IntelGMA] Stolen memory size: %dMB\n", sd->Card.Stolen_size >> 20));
            D(bug("[IntelGMA] Framebuffer window size: %d MB\n", sd->Card.Framebuffer_size >> 20));
            ...
            D(bug("[IntelGMA] Scratch area at %08x\n", sd->ScratchArea));
            ...
            D(bug("[IntelGMA] Using ARGB cursor at graphics address %08x (physical address %08x)\n",
                            sd->CursorImage, sd->CursorBase));
            sd->CursorVisible = FALSE;

            D(bug("[IntelGMA] Initializing CMD ring buffer\n"));
    ...
    //      sd->HardwareStatusPage = (APTR)readl(sd->Card.MMIO + 0x2080);
            D(bug("[IntelGMA] Hardware status page: %08x\n", readl(sd->Card.MMIO + 0x2080)));
            ...
            if( lvds )
            {
                bug("[IntelGMA] lvds Enabled\n");
                sd->pipe = PIPE_B;
                GetSync(sd,&sd->lvds_fixed,PIPE_B); 
            }
            else
            {
                bug("[IntelGMA] analog VGA connector Enabled\n");
                sd->pipe = PIPE_A;
            }
            ...
            AddDisplayDriverA(sd->IntelGMAClass, NULL, NULL);
        }
        else
        {
            D(bug("[IntelGMA] Failed to obtain device, already owned\n"));
        }
    }
...
int G45_Init(struct g45staticdata *sd)
{
    ...
    if (!sd->MemPool)
    {
        bug("[IntelGMA] failed to allocate mempool\n");
        return FALSE;
    }
    ...
    if (sd->tr)
    {
        ...
        {
            D(bug("[IntelGMA] UNIT_MICROHZ of timer.device opened\n"));
            if ((sd->PCIObject = OOP_NewObject(NULL, (STRPTR)CLID_Hidd_PCI, NULL)))
            {
                ...
                if (sd->PCIDevice)
                {
                    D(bug("[IntelGMA] Found supported gfx card\n\003"));
                ...
                D(bug("[IntelGMA] No supported cards found\n"));
