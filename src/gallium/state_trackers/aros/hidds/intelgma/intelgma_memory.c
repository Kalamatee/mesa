    ...
    intptr_t length, BOOL stolen)
{
    ...
    if (success)
    {
        D(bug("[IntelGMA] Mapping system memory %08x to graphics virtual %08x "
...
{
    D(bug("[IntelGMA] Releasing memory block to system (header=%p)\n", header));
    D(bug("[IntelGMA] mh_Lower=%p mh_Upper=%p mh_Free=%p\n",
