/*
    ...
    $Id: compositingclass.c 45935 2012-10-21 12:07:18Z Sami $
*/

/* 
   TODO: Implement hardware assisted compositing...
*/
...
static struct StackBitMapNode * HIDDCompositorCanUseFramebuffer(struct HIDDCompositorData * compdata)
{
	...
	D(bug("[IntelGMA:Compositor] HIDDCompositorCanUseFramebuffer bm %x\n",frn->bm));
    return frn;
}
...
static VOID HIDDCompositorRecalculateVisibleRects(struct HIDDCompositorData * compdata)
{
    ...
    ForeachNode(&compdata->bitmapstack, n)
    {
        ...
        D(bug("[IntelGMA:Compositor] Bitmap %x, visible %d, (%d, %d) , (%d, %d)\n", 
...
static BOOL HIDDCompositorTopBitMapChanged(struct HIDDCompositorData * compdata, OOP_Object * bm)
{
    ...
    */
    ...
    if (compdata->gfx != (OOP_Object *)e)
    {
        ...
        D(bug("[IntelGMA:Compositor] GfxHidd different than one used by compositing\n"));
        return FALSE;
    }
    ...
    {
        D(bug("[IntelGMA:Compositor] Invalid ModeID\n"));
        return FALSE;
    }
    ...
    fbbitmap = HIDD_Gfx_CreateObject(compdata->gfx, CLID_Hidd_BitMap, bmtags);
    if (fbbitmap)
    {
        ...
        if (ret)
        {
            ...
            if (compdata->screenbitmap)
                HIDD_Gfx_DisposeObject(compdata->gfx, compdata->screenbitmap);
        ...
        {
            /* Dispose fbbitmap */
            HIDD_Gfx_DisposeObject(compdata->gfx, fbbitmap);
            return FALSE;
        }
    }
    ...
    OOP_Object * bm, WORD x, WORD y, WORD width, WORD height, BOOL clean)
{
    ...
    if (compdata->screenbitmap)
    {
        ...
        if (AndRectRect(&srcindstrect, &n->screenvisiblerect, &dstandvisrect))
        {
            ...
            {
                /* Clean right side.*/
                D(bug("[IntelGMA:Compositor] clean right %d,%d-%d,%d\n",dstandvisrect.MaxX+1, dstandvisrect.MinY, 
            ...
            {
                /* Clean bottom.*/
                D(bug("[IntelGMA:Compositor] clean bottom %d,%d-%d,%d\n",
...
static VOID HIDDCompositorRedrawVisibleScreen(struct HIDDCompositorData * compdata)
{
	...
	if( ( fbn =  HIDDCompositorCanUseFramebuffer(compdata) ) )
	{
		if( HIDD_INTELG45_SetFramebuffer( fbn->bm ) ){
			D(bug("[IntelGMA:Compositor] directbitmap used, bitmap %x\n",fbn->bm););
	...
	else
	{
		if( compdata->directbitmap ){
			compdata->directbitmap = NULL;
			D(bug("[IntelGMA:Compositor] directbitmap not used, set screenbitmap %x\n",compdata->screenbitmap););
    ...
    if (lastscreenvisibleline > 0)
    {
        ...
        D(bug("[IntelGMA:Compositor] Clean upper area %d,%d-%d,%d\n",0, 0, viswidth - 1, lastscreenvisibleline - 1));
...
OOP_Object *METHOD(Compositor, Root, New)
{
    ...
    if(o)
    {
        ...
        if (compdata->gfx != NULL)
        {
            ...
            compdata->gc = HIDD_Gfx_CreateObject(compdata->gfx, CLID_Hidd_GC, NULL);
        }
...
VOID METHOD(Compositor, Hidd_Compositor, BitMapStackChanged)
{
    ...
    D(bug("[IntelGMA:Compositor] BitMapStackChanged\n"));
    ...
    if (!HIDDCompositorTopBitMapChanged(compdata, msg->data->Bitmap))
    {
        ...
        D(bug("[IntelGMA:Compositor] Failed to change top bitmap\n"));
        UNLOCK_COMPOSITING
        return; 
    }
...
{
    struct HIDDCompositorData * compdata = OOP_INST_DATA(cl, o);
    //D(bug("[IntelGMA:Compositor] BitMapRectChanged\n"));
	if( ! compdata->directbitmap ){
...
{
    struct HIDDCompositorData * compdata = OOP_INST_DATA(cl, o);
    D(bug("[IntelGMA:Compositor] BitMapPositionChanged\n")); 
    LOCK_COMPOSITING_READ
...
VOID METHOD(Compositor, Hidd_Compositor, ValidateBitMapPositionChange)
{
    struct HIDDCompositorData * compdata = OOP_INST_DATA(cl, o);
    struct StackBitMapNode * n = NULL;
      D(bug("[IntelGMA:Compositor] ValidateBitMapPositionChange %d,%d\n",
