...
void SetCursorPosition(struct g45staticdata *sd,LONG x,LONG y)
{
    ...
    {
        width = (sd->VisibleBitmap->state->htotal & 0x0000ffff);
        height = (sd->VisibleBitmap->state->vtotal & 0x0000ffff);
...
{
	bug("[IntelGMA] InitMode %dx%dx%d @ %dHz\n", hdisp, vdisp, depth, ((pixelclock / (uint32_t)htotal) * 1000) / ((uint32_t)vtotal));
	bug("[IntelGMA] InitMode pixclock: %dx\n", pixelclock);
	bug("[IntelGMA] InitMode htotal: %dx\n", htotal);
	bug("[IntelGMA] InitMode vtotal: %dx\n", vtotal);
	...
	if (clock.PixelClock)
	{
		D(bug("[IntelGMA] PixelClock: Found: %d kHz, Requested: %d kHz\n", clock.PixelClock, pixelclock));
		D(bug("[IntelGMA] VCO: %d MHz\n", clock.VCO / 1000));
...
void G45_LoadState(struct g45staticdata *sd, GMAState_t *state)
{
	...
	//	delay_ms(sd, 20);
		...
		/* Clear PIPE status */
		D(bug("[IntelGMA] Old PIPEA Status = %08x\n", readl(sd->Card.MMIO + 0x70024)));
	...
	//		delay_ms(sd, 1);
		...
		D(bug("[IntelGMA] Loaded state. dpll=%08x %08x %08x %08x %08x %08x %08x\n", state->dpll,
...
void GetSync(struct g45staticdata *sd, struct Sync *sync,ULONG pipe)
{
    ...
    switch (dsp_cntr & G45_DSPCNTR_PIXEL_MASK) {
        ...
        case G45_DSPCNTR_32BPP:
            ...
            break;
        default:
            bug("[IntelGMA] GetSync: Unknown pixel format.\n");
    }
    D(
        bug("[IntelGMA] GetSync: %d %d %d %d\n",
                sync->hdisp,
                sync->hstart,
                sync->hend,
                sync->htotal);
